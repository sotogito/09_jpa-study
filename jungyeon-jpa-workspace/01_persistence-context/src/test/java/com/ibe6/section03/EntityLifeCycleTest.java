...
public class EntityLifeCycleTest {
    ...
    public void 영속_find_테스트() {
        ...
        System.out.println("1" + foundedMenu1.hashCode());
        System.out.println("2" + foundedMenu2.hashCode());
        // select 쿼리 한번 실행
        // 근데 나 다른걸로 나옴 ㅎㅎ
    }
    ...
    public void 준영속_detach_테스트() {
        // * detach() : 영속 상태의 엔티티를 분리해서 보관하는 준영속 상태로 변경
        Menu foundedMenu1 = entityManager.find(Menu.class, 20); // db조회 => 영속 상태
        Menu foundedMenu2 = entityManager.find(Menu.class, 21); // db조회 => 영속 상태
    ...
    public void  준영속_close_테스트(){
        // * close() : 영속성 컨텍스트를 종료시키는 메소드, 엔티티매니저를 다시 생성해야만 사용할 수 있음
        Menu foundedMenu1 = entityManager.find(Menu.class, 20); // db조회 => 영속 상태
        Menu foundedMenu2 = entityManager.find(Menu.class, 21); // db조회 => 영속 상태
    ...
    public void 준영속_merge_테스트() {
        ...
        Menu foundedMenu = entityManager.find(Menu.class, 1); // 영속 상태의 객체
    ...
    /*
        ## merge() 기본 동작 원리 ##
        1. merge 메소드 호출시 전달된 준영속 엔티티의 식별자를 가지고 1차 캐시로부터 조회
        2. 1차 캐시에 존재하지 않을경우 DB로 부터 조회해서 1차 캐시에 저장
        3. 조회된 영속 엔티티 객체와 준영속 엔티티 객체 값을 병합 한 뒤 반환
    ...
    public void 준영속_merge_update_테스트(){
        ...
        // foundedMenu (준영속 상태) // 1c차 캐시 제거
        entityManager.detach(foundedMenu);
        foundedMenu.setMenuName("까나리아메리카노"); // 준영속 상태의 객체 값 변경
        Menu mergedMenu = entityManager.merge(foundedMenu);
        // 1) foundedMenu의 식별자(1)를 가지고 1차 캐시로부터 조회 (존재 하지 않아 DB 로부터 주화)
        // 2) DB로부터 조회된 Menu 엔티티와 준영속 상태인 foundedMenu 엔티티와 병합되어 새로운 Menu Entity반환
    ...
    public void 준영속_merge_save_테스트(){
        ...
        Menu mergeMenu = entityManager.merge(foundedMenu);
