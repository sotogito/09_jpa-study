package com.kyungbae.section01.simple;
...
public class SimpleJPQLTest {
    ...
    @Test
    public void 이름기준_파리미터바인딩_테스트(){
        String jpql = "SELECT m FROM menu1 m WHERE m.menuCode = :code";

        Integer menuCode = 8;

        Menu foundedMenu = entityManager.createQuery(jpql, Menu.class)
    ...
    /*
        ...
        1. 그룹 함수의 반환 타입은 정수값을 경우 long, 실수값일 경우 double
        2. 값이 없는 상태에서 count의 결과는 0, count 제외 함수의 결과는 null
            => 반환 값을 담기 위한 변수 선언시 기본자료형으로 하게 되면 언박싱시 NPE 발생 여지가 있음
     */

    @Test
    public void 통계함수_count테스트(){
        String jpql = "SELECT COUNT(m.menuCode) FROM menu1 m WHERE m.categoryCode = ?1";

        Integer categoryCode = 1;

        // auto unboxing => Long 타입 -> long 자료형
        ...
        /*
            ...
            1. 반환타입 Long
            2. 데이터가 없는 경우 0을 반환
            3. 최종 결과를 기록할 변수의 타입은 Long, long(primitive type)을 사용해도 상관없음
         */
