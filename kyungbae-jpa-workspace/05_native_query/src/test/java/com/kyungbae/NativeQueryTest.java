...
public class NativeQueryTest {
    ...
    public void  결과타입명시_테스트(){
        ...
        String sql = """
                ...
                WHERE
                    menu_code = ?
                """;
        Menu foundedMenu = (Menu)entityManager
    ...
    @Test
    public void  결과타입명시x_테스트(){
        String sql = "SELECT menu_code, menu_name FROM tbl_menu";
        List<Object[]> menu = entityManager.createNativeQuery(sql).getResultList();
        menu.forEach(row -> {
    ...
    @Test
    public void 실습1(){
        // 모든 메뉴 조회시 카테고리명도 같이 조회
        String sql = """
                SELECT m.menu_code, m.menu_name, c.category_name
                FROM tbl_menu m JOIN tbl_category c ON c.category_code = m.category_code
                """;
        List<Object[]> menu = entityManager.createNativeQuery(sql).getResultList();
        menu.forEach(row -> {
            System.out.println(Arrays.toString(row));
    ...
    public void 실습2(){
        ...
        String sql = """
                ...
                """;
        List<Object[]> menu = entityManager.createNativeQuery(sql).getResultList();
        menu.forEach(row -> System.out.println(Arrays.toString(row)));
         */

        // 모든 서브 카테고리 조회시 카테고리에 속한 메뉴의 개수를 함께 조회
        String sql = """
                SELECT c.category_code, c.category_name, (SELECT COUNT(*) FROM tbl_menu m WHERE m.category_code = c.category_code) AS menu_count
                FROM tbl_category c
                WHERE ref_category_code IS NOT NULL
                """;
        List<Object[]> menu = entityManager.createNativeQuery(sql).getResultList();
    ...
    @Test
    public void 결과매핑_테스트1(){
        String sql = """
                SELECT c.category_code, c.category_name, c.ref_category_code, (SELECT COUNT(*) FROM tbl_menu m WHERE m.category_code = c.category_code) AS menu_count
                FROM tbl_category c
                WHERE ref_category_code IS NOT NULL
                """;
