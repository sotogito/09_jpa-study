...
public class SimpleJPQLTest {
    ...
    public void LIMIT_활용_테스트(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class)
                                           .setFirstResult(offset)  // LIMIT절의 offset
                                           .setMaxResults(display)  // LIMIT절의 display
                                           .getResultList();
    ...
    /*
        ...
        2. 값이 없는 상태에서 COUNT의 결과는 0, COUNT 제외 함수의 결과는 null
            => 반환값을 담기 위한 변수 선언시 기본자료형으로 하게 되면 언박싱(Wrapper=>Primitive)시 NPE 발생 여지가 있음
     */
    ...
    public void 통계함수_COUNT_테스트(){
        ...
        public void 통계함수_COUNT외_테스트(){
            ...
            String jpql = "SELECT SUM(m.munuPrice) FROM menu1 m WHERE m.categoryCode = ?1";
